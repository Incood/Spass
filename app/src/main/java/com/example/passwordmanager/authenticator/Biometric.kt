...
object Biometric {
    ...
    fun statusName(con: Context): String {
        ...
        result = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            biometricManager.canAuthenticate(BiometricManager.Authenticators.DEVICE_CREDENTIAL or androidx.biometric.BiometricManager.Authenticators.BIOMETRIC_STRONG)
    ...
    fun authenticate(
        ...
        val promptInfo: BiometricPrompt.PromptInfo =
            BiometricPrompt.PromptInfo.Builder().setTitle(title).setSubtitle(subtitle)
                .setDescription(description).setNegativeButtonText(negativeText).build()
